...
def edit_chart_of_account(request):
    ...
    if account_id:
        account_info = ChartOfAccount.objects.filter(id = account_id)
        row = serializers.serialize('json',account_info)
        return JsonResponse({'row':row})
    if request.method == 'POST':
        account_id_edit = request.POST.get('account_id_edit')
        account_title = request.POST.get('account_title')
        account_type = request.POST.get('account_type')
        opening_balance = request.POST.get('opening_balance')
        phone_no = request.POST.get('phone_no')
        ...
        credit_limits = request.POST.get('credit_limit')
        ...
        coa_obj = ChartOfAccount.objects.filter(id = account_id_edit).first()
        coa_obj.account_title = account_title
        coa_obj.parent_id = account_type
        coa_obj.opening_balance = opening_balance
        coa_obj.phone_no = phone_no
        coa_obj.address = email_address
        coa_obj.ntn = ntn
        coa_obj.stn = stn
        coa_obj.cnic = cnic
        coa_obj.remarks = remarks
        coa_obj.credit_limit = credit_limits
        coa_obj.save()
        return JsonResponse({"result":"success"})
    ...
    if request.method == "POST":
        ...
        for value in items:
            ...
            tran1 = Transactions(refrence_id = 0, refrence_date = doc_date, tran_type = '', amount = -abs(amount),
                                date = date, remarks = description, account_id = cash_account,ref_inv_tran_id = invoice_no.id,ref_inv_tran_type = "Purchase CPV", voucher_id = voucher_id )
            tran1.save()
            tran2 = Transactions(refrence_id = 0, refrence_date = doc_date, tran_type = '', amount = amount,
                                date = date, remarks = description, account_id = account_id,ref_inv_tran_id = invoice_no.id,ref_inv_tran_type = "Purchase CPV", voucher_id = voucher_id )
            tran2.save()
            header_id = VoucherHeader.objects.get(voucher_no = get_last_tran_id)
