...
// ===============================================================================
		...
		$(document).on("click", ".delete-bpv", function(){
			...
			$(document).on("click", ".delete-cpv", function(){
				...
				$("#coa_disabled").attr("disabled", "disabled");
				$(".edit_chart_of_account").click(function(){
					$("#coa").attr("disabled", "disabled");
					$("#coa_disabled").removeAttr("disabled");
					...
					req =	$.ajax({
					 ...
					 .done(function done(data){
						 var type = JSON.parse(data.row);
						 console.log(type);
						 console.log(type[0]['pk']);
						 $('#account_id_edit').val(type[0]['pk']);
						 $('#account_title_edit').val(type[0].fields['account_title']);
						 var parent_id = type[0].fields['parent_id'];
						 opening_balance = type[0].fields['opening_balance']
						 if (opening_balance >= 0) {
							 	$( "#debit" ).prop( "checked", true );
	 						 $('#opening_balance_edit').val(type[0].fields['opening_balance']);
						 } else {
							 $( "#credit" ).prop( "checked", true );
		 						 $('#opening_balance_edit').val(Math.abs(type[0].fields['opening_balance']));
						 }
						 $('#phone_no_edit').val(type[0].fields['phone_no']);
						 $('#email_address_edit').val(type[0].fields['email_address']);
						 $('#ntn_edit').val(type[0].fields['ntn']);
						 $('#stn_edit').val(type[0].fields['stn']);
						 $('#cnin_edit').val(type[0].fields['cnic']);
						 $('#address_edit').val(type[0].fields['Address']);
						 $('#remarks_edit').val(type[0].fields['remarks']);
						 $('#credit_limit_edit').val(type[0].fields['credit_limit']);
					 });
					 $('#item_code_jo').val("");
					 $(".add-item-jo").attr("disabled", "disabled");
					 $(".has_id").attr("disabled", "disabled");
				...
				$('.coa_edit').on('submit',function(e){
					e.preventDefault();
					var account_id_edit = $('#account_id_edit').val();
					var account_title = $('#account_title_edit').val();
					var account_type = $('#account_type_edit').val();
					var opening_balance = $('#opening_balance_edit').val();
					var phone_no = $('#phone_no_edit').val();
					var email_address = $('#email_address_edit').val();
					var ntn = $('#ntn_edit').val();
					var stn = $('#stn_edit').val();
					var cnic = $('#cnin_edit').val();
					var address = $('#address_edit').val();
					var remarks = $('#remarks_edit').val();
					var credit_limit = $('#credit_limit_edit').val();
					var op_type = $('input[name=optradio]:checked').val();
						 req =	$.ajax({
								headers: { "X-CSRFToken": getCookie("csrftoken") },
								type: 'POST',
								url : '/transaction/chart_of_account/edit',
								data:{
									'account_id_edit': account_id_edit,
									'account_title': account_title,
